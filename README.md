...
- ⚡ **Built for flow** — **New chat** reuses the current folder instantly (Alt-click to pick another), **↑/↓ recalls your recent prompts**, **↻ Retry** regenerates the last response, **Duplicate** forks a chat, and the **sidebar is drag-to-resize**. A background chat that needs your approval flags itself with an amber dot so nothing silently stalls.
